...
void MeshViewerClass::DrawMesh(bed_mesh_t zval, const uint8_t sizex, const uint8_t sizey) {
  ...
  #define DrawMeshValue(xp, yp, zv) DWINUI::Draw_Signed_Float(font8x16, 1, 2, px(xp) - 18, py(yp) - 6, zv)
  ...
  for (uint8_t y = 0; y < sizey; ++y) {
    ...
    for (uint8_t x = 0; x < sizex; ++x) {
      ...
      if (sizex < 9) {
        if (zmesh[x][y] == 0) DWINUI::Draw_Float(font8x16, 1, 2, px(x) - 12, py(y) - 6, 0);
        else DWINUI::Draw_Signed_Float(font8x16, 1, 2, px(x) - 18, py(y) - 6, zval[x][y]);
      }
      else {
        char str_1[9];
        str_1[0] = 0;
        switch (zmesh[x][y]) {
          case -999 ... -100:
            DWINUI::Draw_Signed_Float(font8x16, 1, 1, px(x) - 18, py(y) - 6, zval[x][y]);
            break;
          case -99 ... -1:
            sprintf_P(str_1, PSTR("-.%02i"), -zmesh[x][y]);
            break;
          case 0:
            DWIN_Draw_String(false, font8x16, DWINUI::textcolor, DWINUI::backcolor, px(x) - 4, py(y) - 6, "0");
            break;
          case 1 ... 99:
            sprintf_P(str_1, PSTR(".%02i"), zmesh[x][y]);
            break;
        ...
        if (str_1[0])
          DWIN_Draw_String(false, font8x16, DWINUI::textcolor, DWINUI::backcolor, px(x) - 12, py(y) - 6, str_1);
      }
    }
  }
}
